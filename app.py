...
# filter_by_asset_class/_ETORO_ASSET_CLASS_TICKERS and find_rotation_candidates/
# execute_rotation used to be defined directly in this file -- extracted out
# to engines/asset_class_utils.py and engines/rotation_engine.py (post-launch
# item #118) with no logic changes. See rotation_engine.py's module
# docstring for why execute_rotation() takes execute_alpaca_trades/
# execute_binance_trades/execute_etoro_trades as parameters instead of
# importing them (those three still live below in this file).
from engines.asset_class_utils import filter_by_asset_class, _ETORO_ASSET_CLASS_TICKERS
from engines.rotation_engine import find_rotation_candidates, execute_rotation
from broker import (
    get_account,
    buy_stock,
    sell_stock,
...
# _ETORO_ASSET_CLASS_TICKERS now lives in engines/asset_class_utils.py
# (imported near the top of this file) -- kept here as a bare reference so
# anyone grepping this file for it lands on the import, not a dead end.
...
# POSITION-CAP ROTATION (manual-approval-first) -- find_rotation_candidates()
# and execute_rotation(), plus their private helpers and the
# ROTATION_MIN_SCORE_GAP/ROTATION_COOLDOWN_HOURS constants, moved to
# engines/rotation_engine.py (post-launch item #118, no logic changed).
# filter_by_asset_class() moved to engines/asset_class_utils.py, imported
# near the top of this file alongside the rotation_engine imports.


# NAME IS STALE (kept as-is to avoid a wider rename): despite the name,
# these asset classes have real eToro broker integration now (see
# execute_etoro_trades() below) -- they only fall back to local paper
# trading when ETORO_LIVE_TRADING is off. Add a new asset class here the
# moment its ASSET_UNIVERSE entry is enabled, and every eToro/paper-
# trading call site below picks it up automatically.
#
# INDICES joined 2026-09-18 (task #390) -- same eToro CFD path as FOREX/
# COMMODITIES. Unlike the SaaS product (saas_decision_engine.py), this
# single-owner bot has no MT4/5 integration at all (mt_broker.py is only
# wired into the SaaS engines), so INDICES here only ever routes through
# eToro, never MT_BRIDGE.
PAPER_ONLY_ASSET_CLASSES = ["FOREX", "COMMODITIES", "INDICES"]
    ...
    for candidate in rotation_candidates:
        ...
        with st.container(border=True):
            ...
            if st.button("✅ Confirm Rotation", key=f"confirm_rotation_{candidate_key}"):
                ...
                execute_rotation(
                    candidate, market_df,
                    execute_alpaca_trades, execute_binance_trades, execute_etoro_trades,
                )
...
if ETORO_LIVE_TRADING:
    for _asset_class in ("FOREX", "COMMODITIES", "INDICES"):
        try:
